"""tests for steps/video/step_00b_whisper.py (mock faster-whisper)"""
...
from steps.video.step_00b_whisper import WhisperStep
...
class TestWhisperStep:
    ...
    def test_validate_missing(self, tmp_path):
        ...
        job_dir.mkdir()
        (job_dir / "input").mkdir()
        config = make_step_config(tmp_path, step_name="00b_whisper")
        step = WhisperStep("00b_whisper", job_dir, config)
    ...
    def test_execute_mock(self, mock_gpu, tmp_path, monkeypatch):
        job_dir = self._setup(tmp_path)
        config = make_step_config(tmp_path, step_name="00b_whisper", pool="gpu")
