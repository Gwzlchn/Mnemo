"""tests for steps/video/step_03_dedup.py (mock imagehash/SSIM)"""
...
class TestDedupStep:
    ...
    def test_validate_missing(self, tmp_path):
        ...
        config = make_step_config(tmp_path, step_name="03_dedup")
        step = DedupStep("03_dedup", job_dir, config)
    ...
    def test_execute_all_identical(self, tmp_path):
        ...
        config = make_step_config(tmp_path, step_name="03_dedup", pool="cpu")
        step = DedupStep("03_dedup", job_dir, config)
        result = step.execute()
