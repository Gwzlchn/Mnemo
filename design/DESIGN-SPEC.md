# Flori 重设计 · 设计规范（DESIGN-SPEC）

> 全站 UI 重设计的实现契约。配合 `prototype.html`（可交互高保真原型）与 `flori.css`（设计系统底座）使用。
...
| `design/flori.css` | 设计 token（CSS 变量）+ 全部组件类 | 直接转成 `tailwind.config` + 基础组件；类名即组件清单 |
...
全部已在 `flori.css` `:root` 落地为 CSS 变量。下表为权威值，迁移到 `tailwind.config.js → theme.extend` 即可。
...
下表组件均已在 `flori.css` 实现并在原型中使用。Vue 实现时一一对应封装。
...
4. **采纳概念（沉淀闭环）**：笔记评审面板或概念库里把候选 `accept` → 它进入知识库工作台「概念」、获得佐证 ★ → 后续笔记 Prompt 用其定义、正文自动把它包成 `term-link`。这是 Flori 的灵魂，UI 要让它顺手可见。
...
*本规范 + `prototype.html` + `flori.css` 三件套自包含，可据此并行开发全站。读笔记是最高频场景，阅读体验与「概念沉淀闭环」的顺手度是验收重点。*
